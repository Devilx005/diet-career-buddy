...
# FRESH HEADER WITH WORKING GREEN LOGIN BUTTON
if st.session_state.authenticated:
    ...
    ">
        ...
        <div style="width: 200px; text-align: right;">
            <button onclick="triggerLoginModal()" style="
    ...
    function triggerLoginModal() {
        // Create and dispatch a custom event to trigger login
...
# Login Modal Trigger (Clean Method)
if not st.session_state.authenticated:
    # Use a simple Streamlit button for clean triggering
    if st.button("🔐 Hidden Login Trigger", key="login_trigger_btn", help="Login trigger"):
        st.session_state.show_login_modal = True
        st.rerun()
    
    # JavaScript to connect header button to Streamlit button
    st.markdown("""
    <script>
    document.addEventListener('openLoginModal', function() {
        // Find and click the Streamlit login trigger button
        const buttons = parent.document.querySelectorAll('button');
        buttons.forEach(btn => {
            if (btn.getAttribute('title') === 'Login trigger') {
                btn.click();
            }
        });
    });
    ...
    .element-container:has(button[title="Login trigger"]) {
        display: none !important;
        position: absolute !important;
        left: -9999px !important;
        visibility: hidden !important;
...
if st.session_state.page == 'tech':
    ...
    # Navigation Buttons with Authentication Check
