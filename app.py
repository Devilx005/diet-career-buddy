...
# FIXED HEADER - Only hamburger for logged in users, positioned LEFT of title
if st.session_state.authenticated:
    ...
    header_text = "🎓 DIET Career Buddy"  # NO hamburger when not logged in
    user_display = ""
    login_button = '<button onclick="document.getElementById(\'login-btn\').click()" style="background: #10a37f; color: white; border: none; padding: 8px 16px; border-radius: 6px; font-weight: 600; cursor: pointer; font-size: 14px;">Log in</button>'
...
<div style="position: fixed; top: 0; left: 0; right: 0; height: 50px; background: #212121; border-bottom: 1px solid #444; display: flex; justify-content: space-between; align-items: center; padding: 0 20px; z-index: 1000;">
    ...
    <div>{login_button}{user_display}</div>
</div>
<div style="margin-top: 60px;"></div>
""", unsafe_allow_html=True)

# ONLY ONE HIDDEN LOGIN BUTTON (not visible)
if not st.session_state.authenticated:
    if st.button("Hidden Login", key="login-btn", help="Login") or st.session_state.show_login_modal:
        st.session_state.show_login_modal = True
        
        # Simple login form - no modal overlay
...
if st.session_state.page == 'tech':
    ...
    # HOME PAGE - Don't show login form here if already showing above
...
# Hidden logout for logged in users
if st.session_state.authenticated:
    if st.button("Logout Hidden", key="logout-hidden", help="Logout"):
        st.session_state.authenticated = False
        st.session_state.username = ""
        st.session_state.page = 'home'
        st.rerun()
